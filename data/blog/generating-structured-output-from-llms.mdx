...
date: '2025-12-30'
lastmod: '2026-02-16'
tags: ['ai', 'llm', 'generative-ai', 'python', 'data-science', 'prompt-engineering']
draft: false
summary: 'A survey of three approaches to generating structured output from LLMs: model fine-tuning, domain-specific language modelling, and schema engineering.'
images: ['/static/images/blog/generating-structured-output/structured-blocks.png']
authors: ['default']
canonicalUrl: 'https://blog.bitwisesolutions.co/blog/generating-structured-output-from-llms'
---

If you have spent any time building applications on top of LLMs, you have probably hit the same wall I did: the model gives you beautiful prose when what you actually need is a JSON object that will not crash your downstream parser. I spent weeks wrestling with retry logic and ad-hoc validation before deciding to properly survey the approaches that existed for coercing structured output from language models.

The ability to consistently generate well-typed output is a key requirement for building operational systems, arguably as important as the text generation capability itself. This article is that survey: an honest assessment of three approaches to generating structured output from LLMs, their trade-offs, and where each one falls short.

> [!NOTE]
> This article was originally written in late 2025. Since then, OpenAI has released native structured output support via `response_format`, and Instructor has become the dominant approach in the Python ecosystem. The conceptual framework and approach comparison remain relevant, but some of the specific library APIs shown may have evolved.

## What is Structured Output

<Image
  src="/static/images/blog/generating-structured-output/structured-blocks.png"
  alt="Structured output blocks diagram"
  width={800}
  height={450}
/>

Structured output refers to data or information generated in accordance with a predefined schema or format. Frequently employed formats encompass JSON, XML, or language-specific constructs like Python dictionaries. These structured outputs may also encompass additional typing information such as [JSON Schema](https://json-schema.org/understanding-json-schema), Pydantic models, or Typescript objects.

> [!TIP]
> When evaluating structured output solutions, keep in mind the two distinct sub-problems: getting valid JSON (schema structure) and getting the right fields with sensible values (schema content). Most solutions handle the first far better than the second.
...
```
Could you please suggest a good recipe for spaghetti bolognese for a family of 4.
Please return a list of ingredients in JSON format and ensure that all measurements
are STRICTLY in grams.
```
...
<Image
  src="/static/images/blog/generating-structured-output/spaghetti.jpeg"
  alt="Spaghetti - illustrating unstructured, messy output"
  width={800}
  height={450}
/>

Imagine if we can reliably coerce the output to the **exact** format that is required. This would remove the need for arbitrary validation logic. With well-structured and predictable responses, integration of the output with other functions or downstream APIs becomes seamless. Ensuring accurate structured outputs stands as a prerequisite in the development of reliable AI applications.

## Approaches to Generating Structured Output

<Image
  src="/static/images/blog/generating-structured-output/spaghetti-maze.jpeg"
  alt="Navigating approaches to structured output"
  width={800}
  height={450}
/>
...
<Mermaid chart={`
graph LR
    subgraph problem["Structured Output"]
        direction TB
        S1["Correct Schema\nStructure"]
        S2["Consistent Schema\nContent"]
    end

    subgraph approaches["Three Approaches"]
        direction TB
        A["Model\nFine-tuning"]
        B["Domain Specific\nLanguage"]
        C["Schema\nEngineering"]
    ...
    subgraph tools["Libraries & Tools"]
        direction TB
        A1["OpenAI Function Calling\nAnthropic Tool Use"]
        B1["LMQL / Guidance\nOutlines / GBNF"]
        C1["Instructor / Marvin\nTypeChat / Zod-GPT"]
    end
...
Let's delve deeper into each approach, exploring their respective advantages and disadvantages. If you would like to run the examples yourself, follow along with the [Colab Notebook](https://colab.research.google.com/drive/11rRaVzRZHbYTfW4tVdbMCULtsVEckQdo?usp=sharing) - you would need to enter an OpenAI API key to run the examples.
...
1. A lower-level approach that operates at the token generation level and constrains the output to match a specific format. This is the approach taken by [parserLLM](https://github.com/r2d4/parserllm) (context-free grammar) and [llama.cpp's GBNF](https://github.com/ggerganov/llama.cpp/blob/master/grammars/README.md) (GGML Bakus-Naur Form) grammar. With a specified grammar, the generating process from an LLM can be precisely constrained to match [JSON](https://github.com/ggerganov/llama.cpp/blob/master/grammars/json.gbnf) or other schemas of interest.[^3] This way, it solves the problem of generating the correct schema. However, this requires underlying access to the model itself and is more applicable to local models.

2. A higher-level approach that operates at the prompt level and allows the user to specify the desired output format. This is the approach taken by [LMQL](https://github.com/eth-sri/lmql), Microsoft's [Guidance](https://github.com/guidance-ai/guidance) and [Outlines](https://github.com/outlines-dev/outlines). All three projects introduce a domain specific language (DSL) that allows a user to interweave programming logic that constrains a model output or additional validation logic to the prompt itself.[^4]
...
[LMQL](https://lmql.ai/docs/latest/language/scripted-prompting.html#escaping) describes itself as a "programming language for large language models (LLMs) based on a superset of Python".

<Image
  src="/static/images/blog/generating-structured-output/LMQL-syntax.png"
  alt="LMQL syntax diagram"
  width={800}
  height={450}
/>

Let's take a look at a sample query using LMQL with OpenAI GPT-3.5:
...
What is more interesting is the logged response. I have shortened it over here for conciseness, but do refer to the [Colab Notebook](https://colab.research.google.com/drive/11rRaVzRZHbYTfW4tVdbMCULtsVEckQdo?usp=sharing) for more details:
  ...
  "events": [
    {
      ...
      "data": {
        "tokenizer": "<LMQLTokenizer 'gpt-3.5-turbo-instruct' using tiktoken>",
        "kwargs": {
...
Being a custom language, LMQL fits quite nicely within the Python ecosystem and offers various advanced functionalities like generating JSON syntax and lists. The [playground](https://lmql.ai/playground/) offers a great way to view the compiled code and execution graph.
...
All the magic happens in these few lines of code. Essentially, it re-writes the dataclass into a sample schema and prompts the LLM to generate the JSON payload which is parsed back to a dataclass.
...
> [!WARNING]
> LMQL's Python-like syntax can be deceptive. While simple examples work well, complex nested structures or custom validation logic can be harder to debug than writing the prompt engineering directly. Consider whether the abstraction layer is earning its complexity for your use case.
...
On the plus side, LMQL is not specific to OpenAI and can be used with any LLMs.[^6] Currently, it supports OpenAI, replicate, and local Llama models via llama.cpp. In fact, without being confined by OpenAI's limitations, such as the maximum 300 logit biases and the restriction on the maximum number of top predictions returned, it might work even better for local models.

## Schema Engineering

<Image
  src="/static/images/blog/generating-structured-output/schema-engineering.png"
  alt="Schema engineering approach diagram"
  width={800}
  height={450}
/>

Another approach that is becoming increasingly popular would be to use type interfaces to prompt and validate text generation. [TypeChat](https://github.com/microsoft/TypeChat) terms it "Schema Engineering". Since type interfaces are widely used in many programming languages, LLMs would already be well-trained on them. They also provide a much more robust interface to reason with.[^7] Instead of dealing with:
...
In the JS/TS ecosystem, there is Microsoft's [TypeChat](https://github.com/microsoft/TypeChat), which uses Typescript's interfaces to generate structured output and [Zod-GPT](https://github.com/dzhng/zod-gpt) which uses [Zod](https://github.com/colinhacks/zod) for parsing and typing. On Python's side, there is [LlamaIndex Pydantic Program](https://gpt-index.readthedocs.io/en/stable/examples/output_parsing/openai_pydantic_program.html), [Marvin](https://github.com/PrefectHQ/marvin) and [Instructor](https://github.com/jxnl/instructor) all of which use [Pydantic](https://github.com/pydantic/pydantic) models to generate structured output.
...
> **Note:** At the time of writing, the libraries referenced below still rely on OpenAI's function calling API, which has recently been deprecated. It is likely that the code will be updated to use the new `tool_choice` argument. Feel free to re-run the [Colab Notebook](https://colab.research.google.com/drive/11rRaVzRZHbYTfW4tVdbMCULtsVEckQdo?usp=sharing) to view the latest results.

### Marvin

At the core of [Marvin](https://github.com/PrefectHQ/marvin) is the `AI Model` component that is identical to Pydantic's `BaseModel`, but uses an LLM to populate the fields. This allows it to transform novel, non-deterministic outputs from LLMs into predictable outputs that can be used to build dependable software.
...
Output: `Location(city='New York City', state='NY')`
...
```json
{
  "messages": [
    {
      "role": "system",
      "content": "The user will provide text that you need to parse into a structured form. To validate your response, you must call the `FormatResponse` function. Use the provided text and context to extract, deduce, or infer any parameters needed by `FormatResponse`, including any missing data. You have been provided the following context to perform your task: - The current time is 2023-11-06 07:39:34.122886+00:00."
    },
    {
      "role": "user",
...
Under the hood, Marvin converts the Pydantic structure into JSON Schema that OpenAI's API expects to receive and forces the model to call the `FormatResponse` function. `FormatResponse` parses the return object back to a Pydantic data model before returning the result.
...
The nice thing about Marvin is that it has quite extensive support for typings including `list`, `Optional`, and imported types like `Datetime`. It also contains other higher-order wrappers like `AI Classifier` and `AI Function` that can be used to simplify prompt logic for specific tasks.
...
Output: `Location(city='New York', state='NY')`
...
Having worked with both libraries, my preference is for Instructor's explicit approach over Marvin's magic. When debugging a failing prompt, I want to see exactly what was sent to the API, and Instructor makes that trivially easy. Marvin's `@ai_model` decorator is elegant, but the hidden system message and function calling setup can make it harder to diagnose when things go wrong. Instructor's approach of patching `ChatCompletion` directly means there is a single, visible point of interaction with the API. What you see is what you send.

## Conclusion

If I were starting a project today that required structured output from an LLM, I would reach for Instructor first. Its Pydantic-native approach gives you the validation, IDE support, and retry logic that the other approaches require you to build yourself. LMQL remains intellectually interesting but has not achieved the ecosystem adoption needed to justify learning a new DSL.[^9] The model fine-tuning approach is now largely a solved problem at the provider level. Both OpenAI and Anthropic offer native structured output modes that handle the schema structure problem well.

What remains unsolved is the schema content problem. No amount of typing will prevent a model from hallucinating a garlic weight of "2 grams" when you meant "2 cloves." That gap between structural correctness and semantic correctness is where the real engineering challenge lies, and where I suspect the next wave of tooling will focus. For now, the combination of provider-side JSON mode and client-side Pydantic validation gets you most of the way there. Just don't trust the numbers without checking them.

May this survey guide you in your journey to build less spaghetti code and more robust AI applications.

---

[^1]: See [JSON mode vs Function Calling](https://community.openai.com/t/json-mode-vs-function-calling/476994/3) forum post - Owen Moore clarified that `JSON mode` is always enabled for the generation of function arguments, but that it does not guarantee that the output will match the schema, only that it can be parsed as valid JSON.

[^2]: I believe function calling is currently implemented by OpenAI and Anthropic.

[^3]: This can be thought of as a more general solution to manipulating logit biases.

[^4]: There are probably even more DSLs out there. [NeMo Guardrails](https://github.com/NVIDIA/NeMo-Guardrails) is a notable one with its own language, Colang, that includes some control flow and validation logic, though the validation logic tends to rely on the LLM itself to evaluate input/output. Since it focuses more on relevancy and safety of output generated, I will not be covering it in the article.
...
[^7]: Credit goes to jxnl for the example.
