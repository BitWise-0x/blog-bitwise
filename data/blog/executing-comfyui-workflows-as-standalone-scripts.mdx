...
I have been using [ComfyUI](https://github.com/comfyanonymous/ComfyUI) extensively for programmatic generative workflows. It is genuinely one of the best tools in the diffusion model ecosystem for prototyping pipelines — the drag-and-drop graph-based interface makes it easy to experiment with different text encodings, generation parameters, and postprocessing steps, and it has deservedly become the power user tool of choice in the developer community.
...
The standalone executor covers the core use case — running a known workflow without the server overhead — but there are several directions I am considering:
