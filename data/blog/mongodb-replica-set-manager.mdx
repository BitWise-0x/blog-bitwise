...
graph TB
    subgraph swarm["Docker Swarm Cluster"]
        subgraph overlay["Encrypted Overlay Network"]
            db1["database<br/><i>MongoDB 8.0</i><br/>Node 1 (Primary)"]
            db2["database<br/><i>MongoDB 8.0</i><br/>Node 2 (Secondary)"]
            db3["database<br/><i>MongoDB 8.0</i><br/>Node 3 (Secondary)"]
        end

        ctrl["dbcontroller<br/><i>Python Sidecar</i><br/>Manager Node Only"]
        ui["nosqlclient<br/><i>Web UI</i><br/>Optional"]
    end

    sock["/var/run/docker.sock"]

    ctrl -- "Docker API:<br/>discover tasks,<br/>inspect IPs,<br/>exec commands" --> sock
    sock --> db1
    sock --> db2
    sock --> db3

    ctrl -. "rs.initiate()<br/>rs.reconfig()<br/>rs.status()" .-> overlay

    db1 <-- "Replication" --> db2
    db1 <-- "Replication" --> db3

    ui -- "mongodb:// connection" --> overlay
